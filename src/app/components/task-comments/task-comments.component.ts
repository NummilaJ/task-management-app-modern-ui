...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ translate('comments') }}</h3>
        ...
        <div class="flex items-start space-x-3">
          ...
          <div class="flex-1">
            <textarea
              ...
              rows="2"
              [placeholder]="translate('addAComment')"
            ...
            <div class="flex justify-between mt-2">
              <small class="text-gray-500 dark:text-gray-400">{{ translate('pressCtrlEnterToSend') }}</small>
              <button
                (click)="addComment()"
      ...
      <div *ngIf="!currentUser" class="mt-4 text-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
        <p class="text-sm text-gray-600 dark:text-gray-400">{{ translate('needLoginToComment') }}</p>
      </div>
    </div>
  `,
  styles: [`
